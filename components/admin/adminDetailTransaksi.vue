...
defineEmits(["closeModal", "changeStatus", "editInvoice", "selectFile"]);

const statusPembayaran = computed(() => {
  switch (props.dataForm.status_sewa) {
    case "belum_bayar":
      return "bg-red-500";
    case "dp":
      ...
      >
        ...
        >
          <h3 class="text-lg text-gray-900 font-semibold dark:text-white">
            {{ capitalizeFirstLetter(stats) }} Invoice
          </h3>
          <button
            type="button"
        ...
        <form
          action="#"
          @submit.prevent="$emit('editInvoice', props.dataForm.kode_invoice)"
        >
          <div class="pb-6">
            <div class="flex justify-between">
              <div class="flex flex-col">
                ...
                <span class="text-sm text-gray-800"
                  >Tgl. {{ toDate(props.dataForm.created_at) }}</span
                >
              </div>
              <div class="self-center">
                ...
                >
                  {{ setStatus(props.dataForm.status_sewa) }}
                </p>
              </div>
            </div>
          </div>
          <div class="flex gap-x-4 pb-4 border-b">
            ...
            <div class="flex flex-col basis-2/3">
              <p class="font-semibold pb-2">Lama Sewa</p>
              <span class="border rounded px-2 py-1">
                {{ toDate(props.dataForm.tanggal_sewa) }} -
                {{ toDate(props.dataForm.tanggal_kembali) }}
              </span>
            </div>
          </div>
        ...
        <div v-if="stats === 'detail'" class="border-t-4 pt-4 mt-4">
          <div class="flex justify-between">
            <p class="text-2xl mb-4">Bukti Pembayaran</p>
            <Icon
              name="bxs:down-arrow"
          ...
          >
            <div v-if="props.dataForm.bukti_dp">
              <p class="pb-2">Bukti DP</p>
              <img
                :src="`${baseApiImg}/${props.dataForm.bukti_dp}`"
                alt=""
                width="300"
                height="300"
            ...
            <div v-if="props.dataForm.bukti_pelunasan">
              <p class="pb-2">Bukti Pelunasan</p>
              <img
                :src="`${baseApiImg}/${props.dataForm.bukti_pelunasan}`"
                alt=""
                width="300"
                height="300"
                class="mr-auto rounded"
            ...
            <div
              v-if="
                props.dataForm.bukti_dp === null &&
                props.dataForm.bukti_pelunasan === null
              "
            >
